...
<!-- Please update using https://github.com/ekalinin/github-markdown-toc. -->

* [1. 導入](#1-導入)
* [2. 例: illegal_access.c](#2-例-illegal_accessc)
* [3. malloc_block チャンク](#3-malloc_block-チャンク)
* [4. 関数と契約](#4-関数と契約)
* [5. パターン](#5-パターン)
* [6. 述語](#6-述語)
* [7. 再帰的な述語](#7-再帰的な述語)
...
* [11. 補題](#11-補題)
* [12. 関数ポインタ](#12-関数ポインタ)
* [13. 参照によるパラメータ](#13-参照によるパラメータ)
* [14. 算術オーバーフロー](#14-算術オーバーフロー)
* [15. 述語族](#15-述語族)
* [16. ジェネリクス](#16-ジェネリクス)
* [17. 述語値](#17-述語値)
* [18. 述語コンストラクタ](#18-述語コンストラクタ)
* [19. マルチスレッド](#19-マルチスレッド)
* [20. 分割所有パーミッション](#20-分割所有パーミッション)
* [21. 正確な述語](#21-正確な述語)
* [22. 自動 open/close](#22-自動-openclose)
* [23. Mutex](#23-mutex)
* [24. リークとダミー断片](#24-リークとダミー断片)
* [25. 文字配列](#25-文字配列)
* [26. 配列に対するループ](#26-配列に対するループ)
* [27. 再帰的なループの証明](#27-再帰的なループの証明)
* [28. 配列の内容物を追跡する](#28-配列の内容物を追跡する)
* [29. 文字列](#29-文字列)
* [30. ポインタの配列](#30-ポインタの配列)
* [31. 練習問題の答](#31-練習問題の答)
...
![図4. if 文のシンボリック実行 (1つ目の場合)。シンボリック実行は if 文の then ブロックを実行します。](img/symexec-if-case1.png)
...
![図8. (if 文をアンコメントした後の) 図1のプログラムの実際の実行木。Chunks を C に、Locals を L に、account を acc に、balance を bal に、myAccount を a に、malloc\_block を mb に省略しています。この木の全てが示されていないことに注意してください: 新しい account のアドレス 𝓁 と balance フィールドの初期値 v があるとき、malloc ノードは (𝓁,v) の組み合わせそれぞれに対して1つの子ノードを持ちます。](img/concrete-execution-tree.png)
...
* もし述語が正確でその入力引数が _X_ 固定であれば、その述語の表明は _X_ において正確です; 出力引数として現れるどのような値も固定になります。例えば、表明 `p(x + y, z)` が正確であると考えられれば、`p` は正確な述語です。さらに、それは1つの入力パラメータを持つと仮定します。すると `x` と `y` は _X_ 中で、その表明は `z` 固定になります。
...
入れ子になった述語の表明の出力引数としてか、もしくは右辺が固定であるような等式の左辺に現れる変数は、固定の変数の集合に追加されます。
...
契約中で `characters` を使うことで、練習問題27を解いてみましょう:
...
関数 `putchars` に同じコマンドを挿入することで、練習問題27への正しい回答が得られます。
...
図10を見てください。このシンボリック実行木と等価な木を表現する数多く方法ががあることに注意してください。特に、1つのシンボリック状態を等価な状態で置き換えることで、等価なシンボリック実行木を作ることができます。もし同じ実際の状態の集合を表現しているのなら、それら2つのシンボック状態は等価です。例えば、もし仮定を満たすシンボル ς に対して唯一の値 v があるなら、ς の出現を v で置換することで等価なシンボリック状態を作ることができます。別の例としては、仮定やヒープチャンク、ローカル変数束縛で挙げられないシンボルがあったら、使われているシンボルの集合からそのシンボルを削除することで等価なシンボリック状態を作ることができます。

![図10. (if 文をアンコメントした後の) 図1のプログラムのシンボリック実行木。。Chunks を C に、Locals を L に、account を acc に、balance を bal に、myAccount を a に、malloc\_block を mb に省略しています。この木の全てが示されていることに注意してください: malloc ノードは2つの子ノードのみを持ち、2つ目の子ノードは実際の実行木で対応する無数の子ノードを集約しています。またローカル変数 a との混乱を避けるため、シンボルを斜体フォントの a で表わしています。](img/symex-tree.png)

__図10. (if 文をアンコメントした後の) 図1のプログラムのシンボリック実行木。。Chunks を C に、Locals を L に、account を acc に、balance を bal に、myAccount を a に、malloc\_block を mb に省略しています。この木の全てが示されていることに注意してください: malloc ノードは2つの子ノードのみを持ち、2つ目の子ノードは実際の実行木で対応する無数の子ノードを集約しています。またローカル変数 a との混乱を避けるため、シンボルを斜体フォントの a で表わしています。__
